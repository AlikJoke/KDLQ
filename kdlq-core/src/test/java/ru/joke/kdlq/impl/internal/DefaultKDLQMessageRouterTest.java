...
public class DefaultKDLQMessageRouterTest {
    ...
    public void testWhenMessageRedeliverySuccess() {
        makeSendingChecks(
                ...
                (sender, message) -> assertTrue(sender.routeToRedelivery(message), "Redelivery should be success"),
                true,
                false
        );
    }
    ...
    public void testWhenMessageRedeliveryAttemptsReached() {
        makeSendingChecks(
                ...
                (sender, message) -> assertFalse(sender.routeToRedelivery(message), "Message should be sent to dlq during redelivery"),
                true,
                true
        );
    }
    ...
    public void testWhenMessageSendingToDLQSuccess() {
        makeSendingToDLQChecks(
                context -> verify(context.lifecycleListener, only()).onMessageKillSuccess(eq(context.producerId), eq(context.originalMessage), eq(context.message)),
                (sender, message) -> assertTrue(sender.routeToDLQ(message), "Sending to DLQ should be success"),
                true);
    }
    ...
    public void testWhenMessageSendingToDLQAttemptsReached() {
        ...
        final KDLQMessageRouter<Object, Object> sender = new DefaultKDLQMessageRouter<>(processorId, config, registry);
        try (sender) {
            assertFalse(sender.routeToDLQ(message), "Sending to DLQ must be skipped");
